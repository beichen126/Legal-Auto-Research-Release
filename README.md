...
1. 下载最新 Release 中的 `reassemble-package.cmd` 和全部 `.part01` 至 `.part13` 文件，并放在同一目录。
2. 双击 `reassemble-package.cmd`。脚本会生成完整 ZIP，并自动校验 SHA-256。
3. 将生成的 ZIP **完整解压到一个全新目录**；不要直接在压缩包预览窗口内运行，也不要只拖出启动文件。
4. 确认解压目录中同时存在 `app`、`runtimes` 和 `workspace` 文件夹。
5. 双击 `LegalAutoResearch.pyw`，或运行 `start.cmd`。首次启动通常需要 1–2 分钟，退出时运行 `stop.cmd`。
...
- Release 使用分卷上传，以避免大文件单连接中断；合并脚本只进行二进制拼接和 SHA-256 校验。
